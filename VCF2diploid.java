...
public class VCF2diploid {
	...
	private String _id = "";
	private boolean _nochains;
	private ArrayList<Variant>[] _variants = new ArrayList[86];

	public VCF2diploid(String[] chrFiles, String[] vcfFiles, String id, boolean pass, boolean nochains, long seed) {
		_chrFiles = chrFiles;
		_vcfFiles = vcfFiles;
		_nochains = nochains;
		if (id != null)
		...
		for (int i = 0; i < vcfFiles.length; i++) {
			...
			System.out.println(vcfFiles[i] + ": " + n_ev + " variants, " + var_nucs + " variant bases");
		}
		...
		for (int f = 0; f < _chrFiles.length; f++) {
			...
			for (int s = 0; s < seqs.length; s++) {
				...
				while (it.hasNext()) {
					...
					if (var.paternal() > 0)
						if (addVariant(paternal_seq, ref_seq, pos, del, var.insertion(var.paternal()), pat_ins_seq)) {
							n_var_pat++;
							n_base_pat += var.variantBases();
						}
					if (var.maternal() > 0)
						if (addVariant(maternal_seq, ref_seq, pos, del, var.insertion(var.maternal()), mat_ins_seq)) {
							n_var_mat++;
							n_base_mat += var.variantBases();
						}
				}

				writeMap(seqs[s], paternal_seq, maternal_seq, pat_ins_seq, mat_ins_seq);
				writeDiploid(ref_seq, paternal_seq, maternal_seq, pat_ins_seq, mat_ins_seq);

				paternal_chains.append(makeChains(seqs[s].getName(), paternalName(seqs[s].getName()), paternal_seq,
						pat_ins_seq, chain_id));
				maternal_chains.append(makeChains(seqs[s].getName(), maternalName(seqs[s].getName()), maternal_seq,
						mat_ins_seq, chain_id));
				chain_id++;

				System.out.println(
						"Applied " + n_var_pat + " variants " + n_base_pat + " bases to " + "paternal genome.");
				System.out.println(
						"Applied " + n_var_mat + " variants " + n_base_mat + " bases to " + "maternal genome.");
			}
		}
	...
	private boolean addVariant(byte[] new_seq, Sequence ref_seq, int pos, int del, String ins,
			Hashtable<Integer, String> ins_seq) {
		boolean overlap = false;

		if (pos >= new_seq.length || pos + del >= new_seq.length) {
			System.err.println("Variant out of chromosome bounds at " + ref_seq.getName() + ":" + (pos + 1)
					+ ", (del,ins) of (" + del + "," + ins + ").");
		...
		for (int p = pos; p < pos + del; p++)
			if (new_seq[p] == DELETED_BASE || new_seq[p] != ref_seq.byteAt(p))
				overlap = true;

		if (overlap) {
			System.err.println("Variant overlap at " + ref_seq.getName() + ":" + (pos + 1) + ", (del,ins) of (" + del
					+ "," + ins + ").");
			System.err.println("Skipping.");
			return false;
		}
		...
		if (del == 1 && ins.length() == 1) { // SNP
			...
			if (ins_seq.get(pos) != null) {
				System.err.println("Multiple insertions at " + ref_seq.getName() + ":" + (pos + 1));
				System.err.println("Skipping variant with (del,ins) of (" + del + "," + ins + ").");
				return false;
			}
			for (int p = pos; p < pos + del; p++)
			...
			if (ins.length() > 0)
				ins_seq.put(new Integer(pos), ins);
		}

		return true;
	}

	private String makeChains(String ref_name, String der_name, byte[] genome, Hashtable<Integer, String> ins_seq,
			int id) {
		...
		wr.println("chain " + score + " " + ref_name + " " + ref_len + " + 0 " + ref_len + " " + der_name + " "
				+ der_len + " + 0 " + der_len + " " + id);
	...
	private void writeMap(Sequence ref_seq, byte[] paternal, byte[] maternal, Hashtable<Integer, String> ins_seq_pat,
			Hashtable<Integer, String> ins_seq_mat) {
		if (paternal.length != maternal.length) {
			System.err.println("Paternal and maternal genomes are of " + "different lengths. Making map aborted.");
			return;
		}
			...
			for (int p = 0; p < paternal.length; p++) {
				...
				if (for_both) {
					...
					if (for_pat) {
						String new_seq = ins_seq_pat.get(p);
						if (new_seq != null) {
							bw.write(NOT_IN_GENOME + "\t" + pi + "\t" + NOT_IN_GENOME);
							bw.newLine();
							for (int i = 0; i < new_seq.length(); i++)
								pi++;
						}
					...
					if (for_mat) {
						...
						if (new_seq != null) {
							bw.write(NOT_IN_GENOME + "\t" + NOT_IN_GENOME + "\t" + mi);
							bw.newLine();
							for (int i = 0; i < new_seq.length(); i++)
								mi++;
						}
				...
				if (maternal[p] != DELETED_BASE)
					mat = mi++;
				if (pr == NOT_IN_GENOME && pp == NOT_IN_GENOME && pm == NOT_IN_GENOME) { // Initiation
					pr = ref;
					pp = pat;
					pm = mat;
				} else {
					boolean cand = ((pat == NOT_IN_GENOME && pp == NOT_IN_GENOME) || ref - pr == pat - pp)
							&& ((mat == NOT_IN_GENOME && pm == NOT_IN_GENOME) || ref - pr == mat - pm);
					if (!cand) {
						bw.write(pr + "\t" + pp + "\t" + pm);
						bw.newLine();
						pr = ref;
	...
	private void writeDiploid(Sequence ref_seq, byte[] paternal, byte[] maternal,
			Hashtable<Integer, String> pat_ins_seq, Hashtable<Integer, String> mat_ins_seq) {
	...
	private void writeGenome(BufferedWriter bw, String name, byte[] genome, Hashtable<Integer, String> ins_seq)
			throws Exception {
	...
	public static void main(String[] args) {
		...
		VCF2diploid maker = new VCF2diploid(chrFiles.toArray(new String[0]), vcfFiles.toArray(new String[0]), id, pass,
				nochains, seed);
		maker.makeDiploid();
	}
